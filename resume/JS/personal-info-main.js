...
$(function () {
    if (config) {
                        ...
                        <div class="card">
                            <div class="row">
                                ...
                                <div class="col-md-9" data-aos="fade-left" data-aos-offset="50" data-aos-duration="500">
                                    <div class="card-body">
                                        <div class="work-description collapsed">${desc}</div>
                                        <button class="read-more-btn"  title="展开阅读完整的工作经历描述">阅读更多</button>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        if (config.others) {
            for (let i = 0; i < config.others.length; i++) {
                ...
                let contentHtml = "";
                // 判断 desc 是否为图片 URL (简单判断是否以 .png, .jpg, .jpeg, .gif 结尾)
                if (desc.toLowerCase().endsWith(".png") || desc.toLowerCase().endsWith(".jpg") || desc.toLowerCase().endsWith(".jpeg") || desc.toLowerCase().endsWith(".gif")) {
                    contentHtml = `<img src="${desc}" alt="${what}" style="max-width: 60%; height: auto; display: block; margin-bottom: 10px;">`;
                } else {
    ...
    function addReadMoreEventListeners() {
        ...
        readMoreButtons.forEach(button => {
            ...
            if (description && description.classList.contains('collapsed')) {
                button.addEventListener('click', function() {
                    console.log("描述文本元素:", description); // 添加这行
                    console.log("初始 classList:", description.classList); // 添加这行
                    description.classList.toggle('expanded');
                    console.log("切换后的 classList:", description.classList); // 添加这行
                    button.textContent = description.classList.contains('expanded') ? '收起' : '阅读更多';
                });
            } else if (button) {
